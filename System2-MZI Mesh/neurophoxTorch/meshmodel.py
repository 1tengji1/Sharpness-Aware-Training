...
class MeshModel:
    ...
    def __init__(self, perm_idx: np.ndarray, hadamard: bool = False, num_tunable: Optional[np.ndarray] = None,
        ...
        self.basis = basis
        self.bs_error_files = bs_error_files if bs_error_files is not None else None
...
#     @property
    def mzi_error_matrices(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        """
#         if not os.path.exists(self.bs_error_files):
#             os.makedirs(self.bs_error_files)
#         e_l_path = os.path.join(self.bs_error_files, 'e_l.npy')
#         e_r_path = os.path.join(self.bs_error_files, 'e_r.npy')
        if self.bs_error_files is None or not (os.path.exists(e_l_path) and os.path.exists(e_r_path)):
            if self.testing:
            ...
            if self.use_different_errors:
                ...
                e_r = e_l
            if self.bs_error_files is not None:
                np.save(e_l_path, e_l)
                np.save(e_r_path, e_r)
        else:
            e_l = np.load(e_l_path)
            e_r = np.load(e_r_path)
        return e_l, e_r
...
#     @property
    def mzi_error_tensors(self):
        ...
        ne_l, ne_r = np.zeros_like(e_l), np.zeros_like(e_r)

        nenn = to_stripe_array(np.sqrt(1 - ne_l) * np.sqrt(1 - ne_r), self.units)
        nepn = to_stripe_array(np.sqrt(1 + ne_l) * np.sqrt(1 - ne_r), self.units)
        nenp = to_stripe_array(np.sqrt(1 - ne_l) * np.sqrt(1 + ne_r), self.units)
        nepp = to_stripe_array(np.sqrt(1 + ne_l) * np.sqrt(1 + ne_r), self.units)

        return [nenn, nepn, nenp, nepp], [enn, epn, enp, epp]
...
class RectangularMeshModel(MeshModel):
    ...
    def __init__(self, units: int, num_layers: int = None, hadamard: bool = False, bs_error: float = 0.0,
                 ...
                 gamma_init_name: str = "random_gamma", bs_error_files: Optional[str] = None):
