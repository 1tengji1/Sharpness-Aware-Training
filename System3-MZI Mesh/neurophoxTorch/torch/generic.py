...
class TransformerLayer(Module):
    ...
    def matrix(self) -> np.ndarray:
        torch_matrix, _ = self.transform(np.eye(self.units, dtype=np.complex64)).cpu().detach().numpy()
        return torch_matrix[0] + 1j * torch_matrix[1]

    @property
    def inverse_matrix(self):
        torch_matrix, _ = self.inverse_transform(np.eye(self.units, dtype=np.complex64)).cpu().detach().numpy()
        return torch_matrix[0] + 1j * torch_matrix[1]
...
class PermutationLayer(TransformerLayer):
    def __init__(self, permuted_indices: np.ndarray):
        ...
        self.permuted_indices = np.asarray(permuted_indices, dtype=np.int64)
...
class MeshTorchLayer(TransformerLayer):
    ...
    def __init__(self, mesh_model: MeshModel, phase_error: float, phase_error_files: str):
        super(MeshTorchLayer, self).__init__(mesh_model.units)
        self.mesh_model = mesh_model
        [nenn, nepn, nenp, nepp], [enn, epn, enp, epp] = self.mesh_model.mzi_error_tensors()
        ...
        self.phase_error, self.phase_error_files = phase_error, phase_error_files
        theta_errors, phi_errors = self.generate_phase_errors()
        self.theta_errors, self.phi_errors = \
                            torch.as_tensor(theta_errors, dtype=torch.float32).cuda(), \
                                torch.as_tensor(phi_errors, dtype=torch.float32).cuda()
        self.nenn, self.nepn, self.nenp, self.nepp = \
        ...
        self.enn, self.epn, self.enp, self.epp = \
    ...
    def generate_phase_errors(self):
        # if not os.path.exists(self.phase_error_files):
        #     os.makedirs(self.phase_error_files)
        # t_e_path = os.path.join(self.phase_error_files, 'theta_errors.npy')
        # p_e_path = os.path.join(self.phase_error_files, 'phi_errors.npy')
        # if self.phase_error_files is None or not (os.path.exists(t_e_path) and os.path.exists(p_e_path)):
        #     t_e = np.random.randn(self.theta.shape[0], self.theta.shape[1]) * self.phase_error
        #     p_e = np.random.randn(self.theta.shape[0], self.theta.shape[1]) * self.phase_error
        #     if self.phase_error_files is not None:
        #         np.save(t_e_path, t_e)
        #         np.save(p_e_path, p_e)
        # else:
        #     t_e = np.load(t_e_path)
        #     p_e = np.load(p_e_path)
    ...
    def transform(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        """
        outputs_cat = []
        for _theta, _phi in zip([self.theta, self.theta + self.theta_errors], 
                                  [self.phi, self.phi + self.phi_errors]):
            mesh_phases = MeshPhasesTorch(
                theta=_theta, phi=_phi, gamma=self.gamma,
                mask=self.mesh_model.mask, hadamard=self.mesh_model.hadamard,
                units=self.units, basis=self.mesh_model.basis
            )
            mesh_layers, mesh_layersE = self.mesh_layers(mesh_phases)
            if isinstance(inputs, np.ndarray):
                inputs = to_complex_t(inputs, _theta.device)
            for _mesh_layers in [mesh_layers, mesh_layersE]:
                outputs = cc_mul(inputs, mesh_phases.input_phase_shift_layer)
                for mesh_layer in _mesh_layers:
                    outputs = mesh_layer(outputs)
                outputs_cat.append(outputs)
        # return No_error, Split_Ratio_Error, Theta_Phi_Error, Both Errors
        return outputs_cat[0], outputs_cat[1], outputs_cat[2], outputs_cat[3]
    ...
    def inverse_transform(self, outputs: torch.Tensor) -> torch.Tensor:
        ...
        for _theta, _phi in zip([self.theta, self.theta + self.theta_errors], 
            ...
            mesh_layers, mesh_layersE = self.mesh_layers(mesh_phases)
            for _mesh_layers in [mesh_layers, mesh_layersE]:
                inputs = to_complex_t(outputs, _theta.device) if isinstance(outputs, np.ndarray) else outputs
                for layer in reversed(range(self.num_layers)):
                    inputs = _mesh_layers[layer].inverse_transform(inputs)
                inputs = cc_mul(inputs, conj_t(mesh_phases.input_phase_shift_layer))
                inputs_cat.append(inputs)
        # return No_error, Split_Ratio_Error, Theta_Phi_Error, Both Errors
        return inputs_cat[0], inputs_cat[1], inputs_cat[2], inputs_cat[3]
    ...
    def mesh_layers(self, phases: MeshPhasesTorch) -> List[MeshVerticalLayer]:
        ...
        mesh_layers_cat = []
        for _enn, _epn, _enp, _epp \
            in zip([self.nenn, self.enn], [self.nepn, self.epn], [self.nenp, self.enp], [self.nepp, self.epp]):

            if self.mesh_model.hadamard:
                s11 = rc_mul(_epp, internal_psl) + rc_mul(_enn, internal_psl.roll(-1, 1))
                s22 = (rc_mul(_enn, internal_psl) + rc_mul(_epp, internal_psl.roll(-1, 1))).roll(1, 1)
                s12 = (rc_mul(_enp, internal_psl) - rc_mul(_epn, internal_psl.roll(-1, 1))).roll(1, 1)
                s21 = rc_mul(_epn, internal_psl) - rc_mul(_enp, internal_psl.roll(-1, 1))
            else:
                s11 = rc_mul(_epp, internal_psl) - rc_mul(_enn, internal_psl.roll(-1, 1))
                s22 = (-rc_mul(_enn, internal_psl) + rc_mul(_epp, internal_psl.roll(-1, 1))).roll(1, 1)
                s12 = s_mul(1j, (rc_mul(_enp, internal_psl) + rc_mul(_epn, internal_psl.roll(-1, 1))).roll(1, 1))
                s21 = s_mul(1j, (rc_mul(_epn, internal_psl) + rc_mul(_enp, internal_psl.roll(-1, 1))))
            ...
            mesh_layers_cat.append(mesh_layers)
        return mesh_layers_cat[0], mesh_layers_cat[1]
...
def s_mul(s: np.complex64, comp: torch.Tensor):
